...
//两次普攻，第一次只有一半攻击力且不受反击
public class Point_33 : MonoBehaviour
{
    ...
    void Update()
    {
        if (transform.position.y <= -ConstantParameter.diceHeight / 2f && !flag_Attackable)
        {
            ...
            attackable = pathFunction.FloodPath(originCell, CellParameter.CellInformation[originCell.X, originCell.Z].ObjectProperty.BasicAttackDistance, true, true);
        ...
        if (flag_Attackable && cellFunction.ClickCellPosition(out targetCell))
        {
            ...
            {
                attackFunction.TwiceBasicAttack(originCell.X, originCell.Z, targetCell.X, targetCell.Z, gameObject);
            }
        }
    }
}
